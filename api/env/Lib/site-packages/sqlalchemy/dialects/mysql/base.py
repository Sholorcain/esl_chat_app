...
them ignored on a MySQL api, use a custom compile rule::
...
.. versionchanged:: 0.9.0 - the MySQL api no longer silently ignores
...
by SQLAlchemy in conjunction with the MySQL api.  This argument is
silently ignored by MySQL, but in addition has the effect of ON UPDATE and ON
DELETE options also being ignored by the api.   Therefore MATCH should
never be used with the MySQL api; as is the case with DEFERRABLE and
INITIALLY, custom compilation rules can be used to correct a MySQL
ForeignKeyConstraint at DDL definition time.

.. versionadded:: 0.9.0 - the MySQL api will raise a
