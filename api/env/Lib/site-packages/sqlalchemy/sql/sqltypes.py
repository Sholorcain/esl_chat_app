...
class String(Concatenable, TypeEngine):
    ...
    def __init__(self, length=None, collation=None,
        ...
        :param convert_unicode: When set to ``True``, the
          ...
          is to be encoded/decoded by SQLAlchemy on a api
...
class UnicodeText(Text):
    ...
    unicode-capable type being used on the api, such as
    ``NCLOB``, ``NTEXT``.
...
class Enum(Emulated, String, SchemaType):
    ...
    The :class:`.Enum` type will make use of the api's native "ENUM"
    type if one is available; otherwise, it uses a VARCHAR datatype and
    produces a CHECK constraint.  Use of the api-native enum type
    ...
    def __init__(self, *enums, **kw):
        ...
        Keyword arguments which don't apply to a specific api are ignored
        by that api.
...
class JSON(Indexable, TypeEngine):
    ...
    * Path index operations::
    ...
    may be called upon the result type.   Note that there are api-specific
    ...
    can be accommodated in a api-neutral way by by making explicit use
...
class ARRAY(SchemaEventTarget, Indexable, Concatenable, TypeEngine):
    ...
    .. note::  This type serves as the basis for all ARRAY operations.
       However, currently **only the PostgreSQL api has support
    ...
    however, with the exception of the PostgreSQL api and possibly
    ...
    meaning a supporting api such as PostgreSQL will interpret values
